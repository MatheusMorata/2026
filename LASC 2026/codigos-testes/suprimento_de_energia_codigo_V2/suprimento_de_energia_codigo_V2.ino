  ...
  if (suprimentoSerial.available()) {
    ...
    if (c == '\n' || c == '\r') {
      if (bufferSerial.length() > 0) {
        ...
        if (idx > 0) {
          // Extrai o endereço do remetente (OBC = 43)
          String srcStr = mensagem.substring(0, idx);
          int srcAddr = srcStr.toInt();
          String comando = mensagem.substring(idx + 1);
          comando.trim();
          
          // Como a comunicação é direta por canal dedicado, se a origem for o OBC (43),
          // ou um broadcast (0), nós processamos o comando imediatamente
          if (srcAddr == 43 || srcAddr == 0 || comando == "REQ_INA") {
