...
// Función de interrupção em RAM para o ESP32-S3
...
// Pinos de comunicação atualizados
...
// Endereços de rede dos subsistemas
...
uint16_t dig_P1; int16_t dig_P2, dig_P3, dig_P4, dig_P5, dig_P6, dig_P7, dig_P8, dig_P9; // CORRIGIDO: Removido int16_t duplicado
int32_t t_fine = 0;
...
// Variáveis Dinâmicas e Buffers Independentes
...
struct __attribute__((packed)) TelemetryPacket {
  ...
  // Dados recebidos do Arduino de Suprimento (INA219)
...
void EE24CXXX::write(unsigned int eeaddress, unsigned char *data, unsigned int data_len) {
  while (data_len > 0) {
    ...
    for (byte i = 0; i < bytesToWrite; i++) Wire.write(data[i]);
...
void EE24CXXX::read(unsigned int eeaddress, unsigned char *data, unsigned int data_len) {
  while (data_len > 0) {
    ...
    for (byte i = 0; i < received; i++) data[i] = Wire.read();
    data += received;
    eeaddress += received;
    data_len -= received;
    if (received == 0) break;
...
EE24CXXX eeprom(0x50); // CORRIGIDO: Agora a classe possui sua estrutura completa
...
void scanI2CAndAssignSensors() {
  DEBUG_SERIAL.println("[I2C] Iniciando varredura forçada...");
  mpuAddress = 0; bmeAddress = 0;
  mpuPresent = false; bmePresent = false;
...
// ============================================
// --- DRIVERS DE REGISTRADORES (NO BRAÇO) ---
// ============================================
bool initMPU6050() {
...
bool readRawMPU(int16_t &ax, int16_t &ay, int16_t &az) {
  if (!mpuPresent) return false;
  uint8_t buf[6];
  if (!i2cReadBytes(mpuAddress, 0x3B, buf, 6)) return false;
...
bool initBME280() {
  ...
  uint8_t calib[24];
  if (!i2cReadBytes(bmeAddress, 0x88, calib, 24)) return false;
...
bool readRawBME(int32_t &adc_P, int32_t &adc_T) {
  if (!bmePresent) return false;
  uint8_t buf[6];
  if (!i2cReadBytes(bmeAddress, 0xF7, buf, 6)) return false;
...
// ============================================
// --- COMUNICAÇÃO SERIAL ENTRE PLACAS -------
// ============================================

void solicitarDadosEscravo(uint8_t destino, String comando) {
  String frame = String(MY_ADDRESS) + ":" + comando;
  ...
  if (loraPacketReceived) {
    ...
    if (state == RADIOLIB_ERR_NONE) {
      ...
      if (len > 2 && rxBuf[0] == DEST_ADDRESS && rxBuf[1] == MY_ADDRESS) {
        ...
        if(cmd == "0" || cmd == "5" || cmd == "6" || cmd == "7" || cmd == "8" || cmd == "10" || cmd == "11" || cmd == "13") {
          String msgMecanismo = String(MY_ADDRESS) + ":" + cmd;
...
void empacotarEEnviarSensores() {
  ...
  if (refPressure == 0.0f && pressure > 0.0f) refPressure = pressure;
  ...
  if ((currentAddress <= 1480) && (altitude >= 500.0f)) {
    eeprom.write(currentAddress, float16((float)seconds)); currentAddress += 2;
    eeprom.write(currentAddress, float16(tempDHT));        currentAddress += 2;
  }
  ...
  pkt.statusFlags = 0;
  if (bmeOkNow) pkt.statusFlags |= FLAG_BME_OK;
  if (mpuOkNow) pkt.statusFlags |= FLAG_MPU_OK;
  if (gpsPresent) pkt.statusFlags |= FLAG_GPS_OK;
  if (dhtOkGlobal) pkt.statusFlags |= FLAG_DHT_OK;
