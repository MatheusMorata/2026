...
#include <SoftwareSerial.h> // Adicionado para gerenciar as seriais extras
#include <Wire.h>
...
// ============================================
#define DEBUG_SERIAL Serial // Usa a porta USB (UART0 equivalente no ESP32-S3)

// 1. GPS em HardwareSerial 1 (Tráfego intenso, melhor no hardware)
HardwareSerial gpsSerial(1); // Usará pinos 45, 46 no setup

// 2. Raspberry em HardwareSerial 2
#define RASP_RX 6
#define RASP_TX 7
HardwareSerial raspSerial(2);

// 3. Controle (Mecanismos) em SoftwareSerial
#define CONTROLE_RX 4   
#define CONTROLE_TX 3   // ATENÇÃO: Alterado de 5 para 3 para evitar conflito com Suprimento
SoftwareSerial controleSerial(CONTROLE_RX, CONTROLE_TX);

// 4. Suprimento em SoftwareSerial
#define SUPRIMENTO_RX 5
#define SUPRIMENTO_TX 18
SoftwareSerial suprimentoSerial(SUPRIMENTO_RX, SUPRIMENTO_TX);
...
void setup() {
    ...
    // Inicialização das Seriais corrigidas
    gpsSerial.begin(9600, SERIAL_8N1, 45, 46);
    raspSerial.begin(9600, SERIAL_8N1, RASP_RX, RASP_TX);
    controleSerial.begin(9600);   // SoftwareSerial usa apenas baud rate
    suprimentoSerial.begin(9600); // SoftwareSerial usa apenas baud rate
    
    DEBUG_SERIAL.println("Seriais configuradas (Sem Conflitos):");
    DEBUG_SERIAL.print("  Controle (Software): RX="); DEBUG_SERIAL.print(CONTROLE_RX); DEBUG_SERIAL.print(", TX="); DEBUG_SERIAL.println(CONTROLE_TX);
    DEBUG_SERIAL.print("  Raspberry (Hardware): RX="); DEBUG_SERIAL.print(RASP_RX); DEBUG_SERIAL.print(", TX="); DEBUG_SERIAL.println(RASP_TX);
    DEBUG_SERIAL.print("  Suprimento (Software): RX="); DEBUG_SERIAL.print(SUPRIMENTO_RX); DEBUG_SERIAL.print(", TX="); DEBUG_SERIAL.println(SUPRIMENTO_TX);
    DEBUG_SERIAL.println("  GPS (Hardware): RX=45, TX=46");
    ...
    delay(2000);
    DEBUG_SERIAL.println("--- Teste de comunicacao ---");
    DEBUG_SERIAL.println("Enviando comando de teste para Controle de Mecanismos...");
...
void executarTelecomando(String cmd) {
    if (cmd == "5") {
        DEBUG_SERIAL.println("-> Parar controle principal");
        enviarOrdemslave(5);
    }
    else if (cmd == "6") {
        ...
        DEBUG_SERIAL.println("-> Emergência (Parar tudo)");
        enviarOrdemslave(10);
    }
    else if (cmd == "11") {
        ...
        DEBUG_SERIAL.println("-> Iniciar missão (Raspberry)");
        enviarOrdemslave(12);
    }
    else if (cmd == "13") {
    ...
    // Comandos de solicitação de dados
        ...
        DEBUG_SERIAL.println("-> Solicitar dados dos Mecanismos");
        solicitarDados('C');
    }
    else if (cmd == "DATA_M") {
        ...
        DEBUG_SERIAL.print("Comando desconhecido ou obsoleto: ");
...
void processarMensagensSuprimento() {
    suprimentoSerial.listen(); // SoftwareSerial precisa de listen para ler buffers
            ...
            if (bufferSuprimento.length() > 0) {
                ...
                if (idx != -1) {
                    ...
                    if (endereco == ADDR_SUPRIMENTO) {
                        ...
                        DEBUG_SERIAL.print("✓ Suprimento: "); DEBUG_SERIAL.println(dados);
                    }
                }
                bufferSuprimento = "";
            }
...
void processarMensagensControle() {
    controleSerial.listen(); // SoftwareSerial precisa de listen para ler buffers
            ...
            if (bufferControle.length() > 0) {
                ...
                if (idx != -1) {
                    ...
                    if (endereco == ADDR_CONTROLE) {
                        ACData = dados;
                        ACDataUpdated = true;
                        DEBUG_SERIAL.print("✓ Controle: "); DEBUG_SERIAL.println(dados);
                    }
                }
                bufferControle = "";
            }
            ...
            if (bufferRasp.length() > 0) {
                ...
                if (idx != -1) {
                    ...
                    if (endereco == ADDR_RASPBERRY) {
                        MData = dados;
                        MDataUpdated = true;
                        DEBUG_SERIAL.print("✓ Missão: "); DEBUG_SERIAL.println(dados);
                    }
                }
                bufferRasp = "";
            }
...
// --- FUNÇÕES AUXILIARES ---
// ============================================
// Ajustado para aceitar tanto HardwareSerial quanto SoftwareSerial (via classe base Stream)
void enviarComandoFormatado(Stream &serialPort, int endereco, String comando) {
...
void enviarOrdemslave(int Buscador) {
    switch(Buscador){
        ...
        default: DEBUG_SERIAL.println("Ordem inválida/Desativada!"); break;
    }
}